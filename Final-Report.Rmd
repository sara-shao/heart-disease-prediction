...
The above 4 categorical variables, sex, chest pain types, exercise-induced angina, and ST slope seem to have some relationship with heart disease incidence, so these variables will be good to look out for in our final model. From the plots above, there appears to be some relationship between heart disease incidence and these four categorical variables, so these variables will be good to look out for in the final results. From the exploratory data analysis, it appears that a positive diagnosis for CVD tends to occur with the sex of a patient being male, asymptomatic chest pain, the presence of exercise-induced angina, and a flat slope of the peak exercise ST segment.
...
### Random Forest:
Here, we fitted a random forest on the data. 
...
We can see in the plot here that 
```{r}
plot(rf.heart)
